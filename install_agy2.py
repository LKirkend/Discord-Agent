"""
File: install_agy2.py
Description:
    Self-contained setup and installation wizard for the Discord Liaison Bot (AGY2) plugin.
...
# Configuration Constants for AGY2 Version
PLUGIN_NAME = "discord-liaison"
DISPLAY_NAME = "Discord Liaison Bot (AGY2)"
DEFAULT_PORT = "18001"
...
PLUGIN_JSON_TEMPLATE = {
  "name": PLUGIN_NAME,
  "version": "1.0.0",
  "description": "Discord Liaison Bot integration for remote approvals, DMs, and agent dashboards in Antigravity 2 (AGY2)",
  "author": {
...
description: "Manage approvals, notifications, dashboards, and conversational DMs for Google Antigravity agents using a Discord bot liaison, configured for Antigravity 2. Use this skill when the user wants to monitor agent sessions, run background agent tasks, or approve shell commands/file edits remotely via Discord buttons."
---

# Discord Liaison Skill (AGY2 version)

This skill integrates a Discord bot into the Google Antigravity SDK to manage remote command approvals and task dashboards for Antigravity 2 (AGY2).
...
"""
        ...
        other_plist_dest = os.path.expanduser("~/Library/LaunchAgents/com.antigravity.discord-liaison-ide.plist")
        if os.path.exists(other_plist_dest):
            log_callback("🧹 Found conflicting Discord Liaison Bot (IDE) launch agent. Unloading and removing it...")
...
def perform_installation(token, user_id, username, client_id, client_secret, gemini_key, port, log_callback):
    ...
    Usage Example:
        perform_installation("token", "user_id", "Tig1", "", "", "", "18001", print)
    """
    try:
