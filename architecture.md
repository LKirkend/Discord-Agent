...
```mermaid
graph TD
    Client[Agent / Client IDE] -->|OpenAI completions /approve| WebServer["src/web_server.py"]
    WebServer -->|Router| ProxyRoutes["src/proxy_routes.py"]
    WebServer -->|UI Render| StatusUI["src/status_ui.py"]
    WebServer -->|Access/Modify| State["src/state.py"]
    
    Bot["src/bot.py"] -->|discord.py Client| DiscordAPI[Discord Server / DM]
    Bot -->|Periodic Updates| Dashboard["src/dashboard.py"]
    Bot -->|Process Checks| Helpers["src/helpers.py"]
    Bot -->|Spawn Actions| AgentManager["src/agent_manager.py"]
    
    DiscordPolicy["src/discord_policy.py"] -->|Pre-approval & Safety Check| Client
    DiscordUI["src/discord_ui.py"] -->|Component Views & Modals| Bot
```
...
### 9. `src/discord_policy.py`
* **Description**: Client-side hook intercepting tool and file modifications to enforce local permissions check.

### 10. `src/discord_ui.py`
* **Description**: Interactive Discord Views, buttons, and text field modals.

### 11. `src/agent_manager.py`
* **Description**: Spawns python tasks as child processes and sends messaging events.

---

## 🧪 Testing Suites
* `tests/test_suite.py`: Master test runner.
* `tests/test_suite_web.py`: Validates FastAPI routes, validation endpoints, and client payloads.
* `tests/test_suite_bot.py`: Tests dashboard renders, daemon checks, and loop schedulers.
* `tests/test_suite_policy.py`: Asserts command filters and user permissions.
